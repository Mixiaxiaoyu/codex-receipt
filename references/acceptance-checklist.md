...
- Estimated or inconsistent token totals are labeled `TOKEN EST.` in the receipt, with `TOKEN COV` showing token-data coverage.
- Machine-readable statistics preserve observed and estimated token fields separately when estimation is used.

## Visual Output

- `receipt-share-final.png` exists and is `1080 px` wide.
- English is the default receipt language.
- `@cn`, `cn`, or `--language zh-cn` renders Chinese receipt labels and persona title while preserving the receipt skin.
...
- User-facing final runs say `ai_illustration_supplied: true`.
...
- Placeholder persona art is used only for automated tests, benchmarks, development demos, or an explicitly accepted fallback.
